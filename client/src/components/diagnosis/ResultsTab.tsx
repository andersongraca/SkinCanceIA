...
export default function ResultsTab({ imagePreview, selectedImage, isProcessing, classificationResult }: ResultsTabProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card><CardHeader><CardTitle>Mapas de explicabilidade</CardTitle><CardDescription>Visualizações aproximadas dos checkpoints reais, restringidas por um gate auxiliar de localização de lesão treinado com ISIC 2016.</CardDescription></CardHeader><CardContent><Alert className="border-blue-200 bg-blue-50"><Info className="h-4 w-4 text-blue-600" /><AlertDescription className="text-blue-800">O gate reduz ativações no fundo, mas os mapas continuam sendo atribuições aproximadas; não são máscaras clínicas nem prova de causalidade.</AlertDescription></Alert>{heatmapItems.length ? <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">{heatmapItems.map(([title, src]) => <div key={src} className="border rounded-lg p-4 space-y-2"><h4 className="font-semibold text-slate-900">{title}</h4><img src={src} alt="" aria-label={title} className="w-full h-64 object-contain rounded bg-slate-100" /></div>)}</div> : <Alert variant="destructive" className="mt-4"><AlertTitle>Falha ao gerar os heatmaps</AlertTitle><AlertDescription>{result.heatmapError || 'O backend não informou a causa. Reinicie o servidor local com o script de diagnóstico e faça um novo upload.'}</AlertDescription></Alert>}</CardContent></Card>
    </div>
  );
}
